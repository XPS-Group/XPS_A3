...
private _fnc_loadFile = {
	params ["_file"];
	private _statement = "createhashmapobject [call compileScript [""%1"",false]];";
...
if (isText _file) then {
	...
	XPS_UT_Engine call ["GetInstance"] call ["AddItem",[_varName,missionNamespace getVariable _varName]];
};
